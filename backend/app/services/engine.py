...
class DeterministicESOPEngine:
    ...
    def check_trustee_holding_period(grant: Grant, check_date: date) -> tuple[bool, date]:
        ...
        **העוגן הוא היום שבו התקיימו *שני* התנאים**, לא אחד מהם. לשון ההגדרה
        ב-102(א) היא "24 חודשים מיום שבו הוקצו המניות **והופקדו** בידי נאמן",
        וההגדרה המשלימה דורשת שההפקדה תתבצע במועד ההקצאה - כלומר המחוקק מניח
        ששני התאריכים זהים. כשהם נפרדים, הקריאה היחידה שיש לה מקור היא
        ``max()``: ספירה מ-``grant_date`` לבדו הייתה מקדימה את תום התקופה, וזה
        אינו הפרש של ימים בסכום אלא **היפוך סיווג** - לפי 102(ב)(4) מימוש לפני
        תום התקופה מסווג את *מלוא* שווי ההטבה כהכנסת עבודה בשיעור שולי במקום
        25%. כלומר המערכת הייתה מדפיסה מספר תקין למראה ונמוך מהחבות האמיתית.

        ``max()`` מפורש ולא ``trustee_deposit_date`` לבדו: ה-API אוסר הפקדה
        לפני ההענקה (routes.py, תיקון ה-backdating של v0.6.0) ולכן היום השניים
        זהים בתוצאה - אבל האילוץ הזה הוא כלל *קלט* שאפשר לשנות, והכלל המסי
        אינו תלוי בו.

        ⚠️ הכלל אומת ב-09/08/2026 מול שכפול הפקודה במאגר חקיקה ושלושה מקורות
        מקצועיים בלתי-תלויים, **לא מול נוסח ראשוני**. שני דברים נותרו חסומים
        ליישום: חלונות ההפקדה המנהליים (45/90 יום) ודין מענק שהופקד באיחור
        ניכר - ראו HANDOFF.md. מענק כזה מקבל כאן תאריך מחושב כרגיל, וזו אינה
        הכרעה שהתקבלה.
        """
        if not grant.trustee_deposit_date:
            return False, check_date

        anchor = max(grant.grant_date, grant.trustee_deposit_date)

        # ROLL_FORWARD בכוונה, ובניגוד ל-cliff: עוגן ב-29/2 מסתיים ב-1/3 ולא

...
from backend.app.models import Company, OptionPool, ShareIssuance
...
def compute_cap_table_snapshot(db: Session, company_id: str, as_of: Optional[date] = None) -> dict:
    ...
    # --- צד המונפק (outstanding) ---
    # סינון עמודה ישיר על company_id + issue_date, לא join/replay - שקול
    # במדויק לשחזור-ledger מלא רק כי ל-ShareIssuance יש כרגע סוג אירוע יחיד
    # (SHARE_ISSUANCE_ESTABLISHED, בלי תיקון/ביטול - ראו models.py.ShareIssuance
    # ו-services/ledger.py::project_share_issuance). אם/כשיתווסף סוג אירוע
    # שני (תיקון/ביטול/העברה), חובה לעבור ל-project() per-row כאן, אחרת
    # החישוב הזה יתעלם בשקט מהתיקון.
    issuance_rows = (
    ...
    for row in issuance_rows:
        ...
        breakdown_by_key[key] = breakdown_by_key.get(key, 0.0) + row.shares
        outstanding_shares += row.shares

    by_shareholder_and_class = [
        {"shareholder_id": shareholder_id, "share_class_id": share_class_id, "shares": shares}

...
עדכון אחרון: **2026-08-13** (v1.0.2 - שני פריטי חוב פתוחים מ-v1.0.1 נסגרו:
איחוד רישום טבלאות ייצוא/ייבוא, וחלון אישור מסמכים פר-סוג-מסמך. `VERSION`
עודכן בקובץ. **טרם הוקם commit** - כל השינויים עדיין ב-working tree בלבד,
ממתינים לבקשה מפורשת נפרדת של המשתתף. ראו הבלוק הבא.)
...
| | |
|---|---|
| גרסה | **v1.0.2 — מומשה ונסקרה (`REPAIR` → תוקן), טרם הוקם commit** |
| תוכנית טכנית | `sunny-toasting-willow.md` |
| `VERSION` | `1.0.2` — בקובץ בלבד. אין תג, אין commit |
| בדיקות | `420 passed, 0 failed` (הייתה `412` בתחילת השיחה) |
| git | working tree עם שינויים לא-committed (11 קבצים משתנים + `company_scope.py` + המיגרציה החדשים). `main` המקומי עדיין על `520b16a` (מיזוג v1.0.1) |
| מיגרציה | `f4b8a2d6e1c9` (`document_acknowledgment_window_overrides`), על גבי `137b929afafc`. אומתה upgrade→downgrade→upgrade על sandbox נפרד. **לא הוחלה על ה-DB החי** |
| DB חי | **ללא שינוי מהבלוק הקודם** - `esop_database.db` נשאר על `137b929afafc`, לא נגעו בו בשיחה הזו |

## הצעד הבא

**v1.0.2 ממתינה להחלטת המשתתף על commit (ואולי push/merge/תיוג בהמשך) -
שום פעולת git לא בוצעה כאן ביוזמה עצמאית, אותו פרוטוקול בדיוק כמו כל גרסה
קודמת.** אחרי ההחלטה הזו, השיחה הבאה פותחת גרסה חדשה (הבאה בתור לפי
`FEATURE_SPEC.md`: v1.1.0 - הערכות שווי) או חוזרת לאחד משני הפריטים
שנדחו למטה, לפי בחירת המשתתף.

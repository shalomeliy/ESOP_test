...
עדכון אחרון: **2026-08-12** (v1.0.0 **שוחררה במלואה.** שלב ב מוזג ל-`main`
(PR #4), ותג `v1.0.0` נוצר ונדחף על ה-merge commit. שני השלבים סגורים,
תויגים, ועל `main`.)
...
| | |
|---|---|
| גרסה | **v1.0.0 — שוחררה במלואה.** שלב א + שלב ב על `main`, מתויגת `v1.0.0` |
| תוכנית טכנית | שלב א: `parallel-jingling-bear.md`. שלב ב: `floating-moseying-hellman.md` |
| `VERSION` | `1.0.0` — ללא שינוי משלב א. תג `v1.0.0` **קיים**, annotated, נדחף ל-`origin` |
| בדיקות | `381 passed, 0 failed` (הייתה `373` בתחילת השיחה שפתחה שלב ב) |
| git | `main` על `9ef5d65` (merge commit של PR #4). `feat/1.0.0/dilution-ui` נשאר קיים ב-remote (לא נמחק, כמו `feat/1.0.0/database` לפניו) |
| מיגרציה | `bd65db40f654` - ללא שינוי בשלב ב (פיצ'ר read-only טהור, אין מיגרציה חדשה) |
| DB חי | ללא שינוי בשלב ב - `esop_database.db` לא נגעו בו בשיחה הזו |

## הצעד הבא

**v1.0.0 סגורה, מוזגת, ומתויגת - אין משימה פתוחה שממתינה להמשך מיידי.**
השיחה הבאה פותחת גרסה חדשה (הבאה בתור לפי `FEATURE_SPEC.md`: v1.1.0 -
הערכות שווי) או עובדת על אחת מנקודות החוב הפתוחות למטה, לפי בחירת
המשתתף - לא ממשיכה את v1.0.0.

...
| `VERSION` | `0.9.1` ✅ |
| בדיקות | `262 passed`, **אזהרה אחת** (של ספרייה חיצונית) |
| git | `main` @ `345d45d` — **יש diff לא מחויב** (ראו למטה) |
| סקירה | ✅ 09/08/2026 על ח1/ח2. סקירת ח3/ח4 + שלוש ההכרעות — ראו למטה |

## הצעד הבא

1. **פיצול `routes.py` לראוטרים לפי תחום** — הוכרע 09/08/2026, ראו למטה.
   **הבא בתור**, לפני שלב ב.
...
**אין כרגע הכרעה פתוחה שממתינה למשתתף.** שלוש ההכרעות שהיו פתוחות
(`EXPIRED`, עוגן 102, `termination_date`) נסגרו ב-09/08/2026.
...
- **`holding_period_end_date` מוחזר כ"היום"** למענק עם נאמן ובלי תאריך הפקדה
  (`routes.py:981`, `:1083`, `:1152`) — קורא כ"התקופה מסתיימת היום" לצד
  `is_met=false`. `documents.py` ו-`notifications.py` מוגנים; אלה לא.

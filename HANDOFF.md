...
עדכון אחרון: **2026-08-14** (v1.1.0 **הושלמה מבחינת תוכן** - חמשת השלבים
בוצעו: DB, backend, frontend, QA, וסקירה עצמאית שחזרה `PASS`. `VERSION`
הועלה ל-`1.1.0`. **כל העבודה של השיחה הזו עדיין ב-working tree, ללא
commit** - שני ה-commits המקומיים הישנים (`05f3a4c`, `c9a4cfc`) הם רק
שכבות ה-DB/backend. נותרו: commit/push/תיוג, והחלת המיגרציה על ה-DB החי -
כולם ממתינים לבקשה מפורשת נפרדת. ראו "הצעד הבא".)

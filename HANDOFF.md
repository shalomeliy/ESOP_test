...
עדכון אחרון: **2026-08-10** (v0.9.1 שלב א + פיצול `routes.py`)
...
| | |
|---|---|
| גרסה | **v0.9.1 — בבנייה.** שלב א הושלם, `routes.py` פוצל, שלב ב (הייצוא) טרם נבנה |
| שלב | **1 מתוך 2** — נאמנות חותמות זמן וסגירת חוב ✅ · פיצול `routes.py` ✅ · ייצוא וניידות ⬜ |
| `VERSION` | `0.9.1` ✅ |
| בדיקות | `264 passed`, **אזהרה אחת** (של ספרייה חיצונית) |
| git | `main` @ `3b5dc31` — **5 commits מקומיים לא-pushed** (ראו למטה). עץ עבודה נקי |
| סקירה | ✅ 09/08/2026 על ח1/ח2. ✅ 10/08/2026 על פיצול `routes.py` (change-reviewer, ראו למטה) |

## הצעד הבא

**שלב ב — הייצוא עצמו.** ייצוא מלא כולל יומן האירועים · סכמה פומבית
JSON+CSV · ייבוא עם dry-run · דוח התאמה · שני מסכים. ראו `FEATURE_SPEC.md`.
פיצול `routes.py` (התנאי המקדים היחיד שהיה) כבר הושלם — ראו למטה.

**לפני שממשיכים:** 5 ה-commits המקומיים טרם נדחפו ל-`origin/main`. `git push`

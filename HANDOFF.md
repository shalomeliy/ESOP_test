...
עדכון אחרון: **2026-08-11** (v0.9.1 שלב ב — 10 מתוך 13 משימות, נסגר באמצע הפיצ'ר)

> **הסגירה הזו שונה מכל הקודמות: היא לא בגבול גרסה.** `tools/context_check.py`
> דיווח `🔴 CLEAN NOW` (510 תורים, ~413k טוקנים מוערכים) באמצע בניית שלב ב.
> בשלב שהתראה הזו נכתבה, הסקירה העצמאית של משימה #10 עדיין לא הייתה החזירה
> פסק דין - היא סיימה מאז: `REPAIR`, שני ממצאים אמיתיים (404 שלא נבדק מול
> הדיסק וקרס ב-500, ושני פערי בדיקה) תוקנו, 332 בדיקות עוברות. **תוכנית
> העבודה המלאה יושבת ב-`PLAN.md`** (13 צעדים מסודרים, §8) — שיחה חדשה חייבת
> לקרוא אותו במלואו, לא רק את הקובץ הזה, כי הוא מכיל את ההחלטות וההכרעות
> שהתקבלו/תוקנו לאורך המשימות 1-10.
...
| | |
|---|---|
| גרסה | **v0.9.1 — בבנייה.** שלב א ✅ (כולל פיצול `routes.py`). שלב ב (ייצוא/ייבוא) **באמצע בנייה: 10/13 משימות** |
...
| git | `main` @ `891bf07` (pushed). משימה 10 (`api/export.py` + `schemas.py` + בדיקות) עדיין לא מחויבת בזמן כתיבת השורה הזו - ראו "לפני שממשיכים" |
| תלות חדשה | `python-multipart==0.0.20` — נוסף ב-`requirements.txt`, נדרש ל-`UploadFile` (ייבוא) |

## הצעד הבא

**משימה #11 (מתוך 13 ב-`PLAN.md` §8): UI - טבלת היסטוריה + מודלים בפורטל
האדמין.** `clients/admin_portal/index_manage.html` (סעיף `<section
id="tab-export">` חדש, לפי צורת `tab-documents`) + `clients/shared/
export_import.js` חדש (מייבא `ESOPDocuments.formatTimestamp`/`.orDash`/
`.errorDetail`/`.downloadDocument`). שני מסכים לפי §5 ב-PLAN.md: היסטוריה
(states `loading→list→error`, `runStatusBadge`, `matchCell` - אייקון
**וגם** טקסט עברי, לעולם לא צבע בלבד) ופאנל פעולה (states `idle→
uploading→dry-run-running→dry-run-shown→committing→done|failed`). ראו
§5 המלא לניסוח ה-copy המדויק ("Done" חייב לפרט מה אומת בפועל).

**משימה #10 נסגרה (endpoints היסטוריה + דוח התאמה).**
שני endpoints חדשים ב-`api/export.py` (אותו router, שום router/מיגרציה
חדשים): `GET /admin/export-import/history` (`?direction=`/`?status=`,
400 על ערך לא מוכר, scoped לפי source_company_id **או**
target_company_id - המסך היחיד שמציג ייצוא+ייבוא יחד) ו-`GET
/admin/export-import/{run_id}/reconciliation` (טוען bundle לפי
`based_on_run_id -> dry_run.file_path`, קורא ל-`reconcile()` בזמן אמת -
אין טבלה/סטטוס שמור ל"טרם הותאם"; 404 אם ה-run אינו `IMPORT_COMMIT` **או**
שה-bundle שלו חסר, 403 חוצה-חברות). שבע בדיקות HTTP-level חדשות
ב-`tests/test_reconciliation.py` (מעל תשע קיימות מ-task #9 - 16 בקובץ).
פירוט מלא, כולל ההכרעה על "טרם הותאם", ב-`PLAN.md` ("Implementation
notes added during task #10").

**אחרי #11:** #12 (עדכון `docs/qa/v0.9.1.md`) · #13 (עדכון `VERSION`).

**לפני שממשיכים:** ודאו ש-`git status` נקי ו-`origin/main` מעודכן -
משימה 10 (כולל תיקוני הסקירה העצמאית) הייתה עדיין לא מחויבת כשהשורה
הזו נכתבה. `git push` דורש אישור מפורש מהמשתתף בכל שיחה - לא מונח מראש.

## מה נבנה ב-10-11/08/2026 (שלב ב, משימות 1-10 מתוך 13)

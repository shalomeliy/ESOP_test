...
עדכון אחרון: **2026-08-13** (v1.1.0 **באמצע בנייה, שיחה לא סגורה** - תכנון
מלא אושר, ה-DB layer וה-backend layer מומשו ואומתו ו-**נדחפו ל-commit
מקומי** (לא ל-`origin`), אבל frontend/QA/סקירה עצמאית/אימות ידני עדיין לא
בוצעו. השיחה הבאה ממשיכה בדיוק מכאן - ראו "הצעד הבא" למטה. `VERSION` לא זז
(עדיין `1.0.2` - יזוז רק בסגירת הגרסה).)
...
| | |
|---|---|
| גרסה | **v1.1.0 — באמצע בנייה.** DB layer + backend layer הושלמו, frontend/QA/סקירה/אימות עדיין לא |
| תוכנית טכנית | `reactive-frolicking-cherny.md` |
| `VERSION` | `1.0.2` — **לא זז עדיין**, יזוז רק בסגירת v1.1.0 |
| בדיקות | `420 passed, 0 failed` — ללא שינוי מהבסיס, אין רגרסיה משני השלבים שבוצעו |
| git | `main` מקומי לפני `origin` ב-2 commits (`05f3a4c` DB, `c9a4cfc` backend), **לא נדחפו** |
| מיגרציה | `23416d683773` (`saved_reports`), על גבי `f4b8a2d6e1c9`. **לא הוחלה על ה-DB החי** — רק אומתה על סנדבוקס נפרד |
| DB חי | **ללא שינוי.** `esop_database.db` עדיין על `f4b8a2d6e1c9`, לא נגעו בו בשיחה הזו |

## הצעד הבא

**v1.1.0 באמצע בנייה, שיחה נסגרה ביוזמת המשתתף (context גדל) - יש בדיוק
משימה פתוחה שממשיכים ממנה, לא פתיחת גרסה חדשה.** התוכנית המלאה
(`reactive-frolicking-cherny.md`) מפרטת חמישה שלבים; שניים הושלמו (DB,
backend), שלושה נותרו, **לפי הסדר הזה**:

1. **Frontend** (frontend-engineer) — טאב חדש בפורטל האדמין (`clients/
   admin_portal/index_manage.html`), `clients/shared/reports.js`, שני
   גרפים ללא ספריית צ'ארטינג (stacked-bar לפילוח מס, SVG polyline ל-עקומת
   הבשלה עם `dir="ltr"` מפורש), מסנן טווח-תאריכים, UI לדוחות שמורים, badge
   "אומדן" (נפרד מ-"לא זמין" הקיים). ה-API כבר מוכן ומאומת (`GET /admin/
   reports/*`) - זו עבודת UI טהורה מעכשיו.
2. **QA** (qa-engineer) — `tests/test_reports.py` (בידוד חוצה-חברות לכל
   endpoint, קצוות טווח-תאריכים, חלק Trustee בדוח התנועה, הגנת CSV,
   נראות דוחות שמורים, דוגמה מחושבת-ביד לפחות לדוח הוצאות-תגמול ולדוח
   חשיפה-לפי-נאמן), `docs/qa/v1.1.0.md` חדש מ-`_TEMPLATE.md`, עדכון
   `QA_TESTBOOK.md`, ותוספת ל-`test_project_invariants.py` אם נדרש.
3. **סקירה עצמאית** (change-reviewer) על כל הדיף, לפני שסוגרים.

**אחרי אלה**: אימות ידני מקצה-לקצה מול `qa_sandbox.db` (לא ה-DB החי), ורק
אז — בבקשה מפורשת נפרדת של המשתתף כמו תמיד — bump ל-`VERSION`, commit,
push, ותיוג.

**למה זה לא "המשך אותה שיחה"**: המשתתף ביקש במפורש לעצור כאן ולפתוח שיחה
חדשה אחרי שה-DB+backend נסגרו ב-commit מקומי, כי ה-context כבר גדול. אותו
תקדים בדיוק כמו v1.0.0 שלב א' (שנסגר עם 6 commits מקומיים, לא נדחפו, ושלב ב'
המשיך בשיחה נפרדת).

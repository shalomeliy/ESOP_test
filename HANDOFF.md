...
עדכון אחרון: **2026-08-11** (v1.0.0 שלב ב — **הושלם. חישוב דילול + UI
לטבלת ההון בנוי ונבדק. שני השלבים של v1.0.0 סגורים - מוכן לתיוג, בכפוף
למיזוג הענף הזה.**)
...
>
> **`feat/1.0.0/dilution-ui`** (ענף חדש, מ-`main` מעודכן) - **commits עדיין
> לא נוצרו/נדחפו בסוף השיחה הזו** (ראו "הצעד הבא"). `VERSION` **לא** זז
> (נשאר `1.0.0`, כמו שנקבע בסוף שלב א) ותג `v1.0.0` **לא** נוצר עדיין -
> אלה תפקיד release-manager, בכוונה לא בוצעו בשיחה הזו.
...
| | |
|---|---|
| גרסה | **v1.0.0 — שלב א ✅ + שלב ב ✅ הושלמו (מודל דאטה+ledger+API, ואז דילול+UI). מוכן לתיוג אחרי מיזוג הענף.** |
| תוכנית טכנית | שלב א: `parallel-jingling-bear.md`. שלב ב: `floating-moseying-hellman.md` |
| `VERSION` | `1.0.0` — ללא שינוי משלב א (`test_version_file_is_not_behind_the_qa_testbook` כבר עבר). תג `v1.0.0` **לא** נוצר - תפקיד release-manager, אחרי מיזוג `feat/1.0.0/dilution-ui` |
| בדיקות | `381 passed, 0 failed` (הייתה `373` בתחילת השיחה) |
| git | ענף `feat/1.0.0/dilution-ui` (מ-`main` מעודכן) - **קבצים מוכנים, commit עדיין לא בוצע** בסוף השיחה הזו (ראו "הצעד הבא") |
| מיגרציה | `bd65db40f654` - ללא שינוי בשלב ב (פיצ'ר read-only טהור, אין מיגרציה חדשה - `git diff origin/main -- backend/app/models.py database/init_scheme.sql` ריק) |
| DB חי | ללא שינוי בשלב ב - `esop_database.db` לא נגעו בו בשיחה הזו |

## הצעד הבא

**שני השלבים של v1.0.0 סגורים. השיחה הבאה פותחת ב-commit + push של
`feat/1.0.0/dilution-ui`, ואז PR + מיזוג ל-`main` + תג `v1.0.0`** - בדיוק
התהליך שכבר קרה לשלב א (`gh pr create` → `gh pr merge --merge`, ללא
reviewer שני מהסיבה שתועדה שם - `shalomeliy` הוא ה-collaborator היחיד).
לא בוצע כאן במכוון - commit/push/merge/תיוג הם פעולות שדורשות אישור
מפורש בכל פעם, לא רק פעם אחת לכל הפרויקט.

**נקודת חוב אחת נשארת פתוחה מפורשות** (שתי הנקודות האחרות שהיו רשומות
כאן - תכנון שלב ב, ו-DB חי בסטייה - **נסגרו**, הראשונה בשיחה הזו והשנייה
ב-11/08/2026 בסבב הקודם):

1. **ייצוא/ייבוא (v0.9.1) לא כולל את טבלאות ההון החדשות.** `ShareClass`/
   ...
   שלב א/ב במפורש, לא חוב שנצבר בשקט.

**שתי נקודות חוב חדשות, שנוצרו במפורש בשלב ב (לא נצברו בשקט):**

2. **ניתוק בין מימוש אופציות לטבלת ההון.** אישור `ExerciseRequest` לא יוצר
   ...
   ב-`docs/qa/v1.0.0.md`.
3. **`ShareIssuance` נשאר בעל סוג אירוע `ledger` יחיד** (סיכון 3, ללא שינוי
...
(✅ עכשיו). **commit + push עדיין לא בוצעו** - ראו למעלה, זו המשימה
הראשונה של השיחה הבאה (או של תור נוסף בשיחה הזו, אם המשתתף מאשר עכשיו).

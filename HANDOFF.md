...
**שתי נקודות חוב חדשות, שנוצרו במפורש בשיחה הזו (לא נצברו בשקט):**

- **שני מבנים מקבילים (`export.py::TABLE_REGISTRY` מול `import_.py::_TABLE_SPECS`/
  `_MODEL_BY_TABLE`) עדיין לא אוחדו.** זה מה שאיפשר לפער המקורי של פריט 1
  לקרות מלכתחילה - הרחבה עתידית יכולה לשכוח לעדכן צד אחד בדיוק כמו שקרה
  כאן. איחוד לתיאור-טבלה משותף יחיד נדחה בכוונה מגרסת ה-patch הזו (שינוי
  ארכיטקטוני, לא תיקון-באג) - סיכון 3 ב-`docs/qa/v1.0.1.md`.
- **חלון אישור מסמכים נשאר פר-חברה, לא פר-סוג-מסמך.** נדון בתכנון ונדחה
  בכוונה - אין צורך מוצהר לגרנולריות עדינה יותר כרגע. סיכון 6 ב-`docs/qa/v1.0.1.md`.

**נקודות חוב ישנות שלא נגענו בהן, נשארות פתוחות בדיוק כמו קודם:**

1. **ניתוק בין מימוש אופציות לטבלת ההון.** אישור `ExerciseRequest` לא יוצר
   `ShareIssuance`/`Shareholder` - הכרעה מוצרית מפורשת (ראו למעלה, החלטה 2),
   לא תקלה. אם גרסה עתידית תרצה שעובד שמימש יופיע כבעל-מניות בפועל, נוסחת
   ה-fully-diluted (החלטה 1) חייבת להיבחן מחדש כדי לא לספור פעמיים. סיכון 8
   ב-`docs/qa/v1.0.0.md`.
2. **`ShareIssuance` נשאר בעל סוג אירוע `ledger` יחיד** (סיכון 3, ללא שינוי
   בהחלטה) - אבל `compute_cap_table_snapshot` הוסיף עליו תלות חדשה: הסינון
   הישיר (`issue_date <= as_of`) שקול ל-replay מלא רק כל עוד הסיכון הזה
   פתוח. אם/כשיתווסף סוג אירוע שני (תיקון/ביטול/העברה), חובה לעבור ל-
   `project()` per-row ב-`services/cap_table.py`, אחרת חישוב הדילול יתעלם
   בשקט מהתיקון.

**לקח מהשיחה הזו, שווה לזכור בפעם הבאה שמוסיפים מכנה לחלוקה:** `is not
None` אינו שקול ל-"ערך תקין לחלק בו" - `0.0` הוא ערך ממשי שעובר את הבדיקה
ומפיל `ZeroDivisionError`. נתפס בסקירה עצמאית (`change-reviewer`) **ועל ידי
בנפרד**, לא הונח מלכתחילה בתכנון - ראו את הסעיף המלא למעלה.

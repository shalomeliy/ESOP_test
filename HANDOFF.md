...
עדכון אחרון: **2026-08-11** (v0.9.1 שלב ב — 6 מתוך 13 משימות, נסגר באמצע הפיצ'ר)
...
> הזה, כי הוא מכיל את ההחלטות וההכרעות שהתקבלו/תוקנו לאורך המשימות 1-6.
...
| | |
|---|---|
| גרסה | **v0.9.1 — בבנייה.** שלב א ✅ (כולל פיצול `routes.py`). שלב ב (ייצוא/ייבוא) **באמצע בנייה: 6/13 משימות** |
...
| בדיקות | `300 passed`, אזהרה אחת (ספרייה חיצונית) |
| git | `main` @ `547f231` — **4 commits חדשים היום, + 5 קודמים = 9 מקומיים לא-pushed**. עץ עבודה נקי |
| תלות חדשה | `python-multipart==0.0.20` — נוסף ב-`requirements.txt`, נדרש ל-`UploadFile` (ייבוא) |

## הצעד הבא

**משימה #7 (מתוך 13 ב-`PLAN.md` §8): commit path של הייבוא.**
`services/import_.py::commit()` — כתיבה בפועל בסדר טופולוגי (§3), אכיפת
`company_id` בצד השרת על כל שורה (לא לוקח מהקובץ), אידמפוטנטיות של
`LedgerEvent` לפי `(aggregate_id, sequence_no)` (לא `event_id`), וחישוב
מחדש חד-פעמי של פרויקציות (`OptionPool.allocated_shares` וכו') אחרי כל
הבאטש, לא אירוע-אירוע. `dry_run()` (משימה 6, כבר קיים) הוא נקודת המוצא -
`commit()` מריץ אותו מחדש מול הקובץ שנשמר (לא סומך על דוח ישן), ורק אם
תקין כותב בפועל.

**אחרי #7:** #8 (שני שלבים dry-run→commit, 409 על dry-run מיושן/מנוצל) ·
#9 (דוח התאמה שמריץ מחדש את מנועי ההבשלה/מס, לא רק משווה ספירות) · #10
(endpoints היסטוריה+התאמה) · #11 (UI בפורטל האדמין) · #12 (עדכון
`docs/qa/v0.9.1.md`) · #13 (עדכון `VERSION`).

**לפני שממשיכים:** 9 commits מקומיים טרם נדחפו ל-`origin/main`. `git push`
דורש אישור מפורש מהמשתתף בכל שיחה — לא מונח מראש.

## מה נבנה ב-10-11/08/2026 (שלב ב, משימות 1-6 מתוך 13)

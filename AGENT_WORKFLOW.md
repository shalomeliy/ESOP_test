...
6. **הבאגים המכוונים תוקנו** (באישור מפורש). `QA_TESTBOOK.md` הוא מסמך הבדיקות החי,
   ומתעדכן בכל גרסה — סגירת גרסה בלי עדכון שלו היא גרסה לא גמורה. אין להחזיר באגים
   לקו המוצר; ההיסטוריה שמורה בתג `qa-buggy-baseline-v1`.
